	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				// Изменение цены в зависимости от группы пользователя
				$customer_id = isset($this->session->data['customer_id']) ?  $this->session->data['customer_id'] : 0;

				if($customer_id  != 0) {
					$customer_group_id = $this->db->query("SELECT customer_group_id FROM " . DB_PREFIX . "customer WHERE customer_id = " . $customer_id);

					$customer_group_id = $customer_group_id->row['customer_group_id'];
				} else {
					$customer_group_id = 1;
				}
				...
				if($customer_group_id == 2) {
					$res = $this->db->query("SELECT price_3 FROM " . DB_PREFIX . "prices WHERE product_id = " . $product_query->row['product_id']);
					if(isset($res->row['price_3'])) {
						$price = $res->row['price_3'];
					} else {
						$price = $product_query->row['price'];
					}
				...
				if($customer_group_id == 3) {
					$res = $this->db->query("SELECT price_4 FROM " . DB_PREFIX . "prices WHERE product_id = " . $product_query->row['product_id']);
					if(isset($res->row['price_4'])) {
						$price = $res->row['price_4'];
					} else {
						$price = $product_query->row['price'];
					}
				}
