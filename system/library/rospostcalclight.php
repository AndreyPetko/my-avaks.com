...
/**
 * @remake functions => class (a@larionov.me), 02/2016
 *
 *
 *
...
class RospostCalclight {
	
	private $config_vars = array('source', 'st',	'ml', 'ib', 'r', 'pr', 'pk', 'cs', 'd', 'default_from', 'hide_from', 'hide_to', 'autocomplete_items', 'log', 'error_log', 'error_log_send', 'cache_dir', 'cache_valid', 'timeout', 'servers', 'city_as_pindex', 'allow_detail_types', 'allow_detail_title', 'show_cod', 'append_cod_to_total','allow_part_package','type', 'only_costtype', 'ico_type', 'ico_size', 'ico_pos');
	private $db;
	private $config;
	...
	function postcalc_get_config() {
		foreach ($this->config_vars as $key) {
			$this->{"config_$key"}  = $this->config->get('rospost_postcalc_' . $key);
		}

		// cache dir absolute path
		$this->config_cache_dir_path = (file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . $this->config_cache_dir) ? $_SERVER['DOCUMENT_ROOT'] . '/' . $this->config_cache_dir : sys_get_temp_dir());
		// list servers
		$this->config_servers = explode(',', $this->config_servers);
	}
	...
	function postcalc_request($From, $To, $Weight, $Valuation = 0, $Country = 'RU') {
		...
		if(is_array($PindexFrom) && isset($PindexFrom['error']))
			return $PindexFrom['error'];
		...
		$QueryString .= "&o=php&sw=PostcalcLight_2.0&cs=$this->config_cs";
		...
		$CacheFile = "$this->config_cache_dir_path/postcalc_".md5($QueryString).'.txt';
		// Сборка мусора. Удаляем все файлы, которые подходят под маску, старше POSTCALC_CACHE_VALID секунд 
		$arrCacheFiles = glob( "$this->config_cache_dir_path/postcalc_*.txt" );
		$Now = time();
		...
		if ( file_exists($CacheFile) ) {
			...
			$arrOptions = array('http' =>
			  array( 'header'  => 'Accept-Encoding: gzip',
					 'timeout' => $this->config_timeout, 
					 'user_agent' => 'PostcalcLight_2.0 '.phpversion() 
				   )
			);
			$TS=microtime(1);
			...
			foreach ( $this->config_servers as $Server ) {
				...
				if ( !$Response = file_get_contents("http://$Server/?$QueryString", false , stream_context_create($arrOptions)) ) {
					  ...
					  if ( $this->config_error_log && count(error_get_last()) ) {
							$ErrorLog = "$this->config_cache_dir_path/postcalc_error_" .date('Y-m') .'.log';
			...
			if ( !$arrResponse = unserialize($Response) ) 
					return "Получены странные данные. Повторите попытку позже."; //Ответ сервера:\n$Response
			...
			if ( $this->config_log ) {
				$fp_log = fopen("$this->config_cache_dir_path/postcalc_light_" .date('Y-m') .'.log','a');
	...
	function postcalc_arr_from_txt($src_txt, $search = '', $limit = 0){
		...
		if ( $this->config_source == 'mysql' ) {
			...
			$s = $this->db->query("SHOW TABLES LIKE '" . $TableName . "'");
			if(!$s->num_rows)
				return array('error' => 'БД не готова. Проверьте наличие необходимых таблиц.');
			...
			$stmt = $this->db->query("SELECT * FROM $TableName $Where $Order $Limit");
			foreach ($stmt->rows as $rr) {
				foreach ($rr as $v)
					$g[] = $v;
				 $arr[$g[0]] = $g[1];
			}
	...
	{
		$Charset = $this->arrPostcalcConfig['cs'];
		$arr = array();
	...
	function postcalc_get_stat_arr() {
		$arrStat=array();
		foreach (glob("$this->config_cache_dir_path/postcalc_light_*.log") as $logfile ) {
		...
		foreach (glob("$this->config_cache_dir_path/postcalc_error_*.log") as $logfile ) {
			$fp_log=fopen($logfile,'r');
			while ( $logline = fgets($fp_log)) {
