...
class ControllerModuleOccm extends ECController12 {
	...
	public function index() {
		...
		$this->template = 'default/template/module/occm.tpl';
			...
			if (!$json) {
				if (empty($this->request->post['occ_customer']) && empty($this->request->get['occ_customer'])) {
					...
					if (2 == $this->c('occm_name_field') && empty($this->request->post['occ_customer']['firstname'])) {
						...
						$mainCart = $this->session->data['cart'];
						$this->cart->clear();
						...
						$this->session->data['cart'] = $mainCart;
					}
	...
	public function js() {
		...
		$this->template = 'default/template/module/occm_js.tpl';
	...
	protected function mOrder($customer) {
		...
		if (defined('VERSION') && '2.' == substr(VERSION, 0, 2)) {
			...
			$results = $this->m('setting/extension')->getExtensions('total');
		}
		...
		if (defined('VERSION') && '2.' == substr(VERSION, 0, 2)) {
			...
			$this->m('checkout/order')->confirm($orderId, $this->c('occm_order_status_id'));
		}
		return $orderId;
	}
}
