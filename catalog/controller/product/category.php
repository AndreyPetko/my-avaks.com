...
class ControllerProductCategory extends Controller {
	...
	public function index() {
		...
		$data['show_filter'] = $category_info['parent_id'] ? 1 : 0;
		...
		if ($category_info) {
			...
			$filtersIds = $this->db->query("SELECT fd.filter_id, fd.name, f.filter_group_id FROM oc_category_filter as cf
				LEFT JOIN oc_filter as f ON cf.filter_id = f.filter_id
				LEFT JOIN oc_filter_description as fd ON cf.filter_id = fd.filter_id
				LEFT JOIN oc_filter_group_description as fgd ON f.filter_group_id = fd.filter_group_id
				WHERE category_id = " . $category_id . " GROUP BY f.filter_id");

			$groups = $this->db->query("SELECT DISTINCT(f.filter_group_id), fgd.name  FROM oc_category_filter as cf
				LEFT JOIN oc_filter as f ON cf.filter_id = f.filter_id
				LEFT JOIN oc_filter_group_description as fgd ON f.filter_group_id = fgd.filter_group_id
			...
			foreach ($groups->rows as $item) {
				foreach ($filtersIds->rows as $elem) {
					if($item['filter_group_id'] == $elem['filter_group_id']) {
						$checked = 0;
						foreach ($filterArr as $filterId) {
							if($filterId == $elem['filter_id']) {
								$checked = 1;
							}
						}
			...
			$data['groupsFilter'] = $groupIds;


			if($filter) {
				$data['checked_filter'] = $this->db->query("SELECT filter_id as id,name FROM oc_filter_description WHERE filter_id IN (" . $filter . ")")->rows;
			}



			$prices = $this->db->query("SELECT DISTINCT(oc_product.price) FROM oc_product_to_category
				LEFT JOIN oc_product ON oc_product_to_category.product_id = oc_product.product_id
				WHERE category_id = " . $category_id . " AND oc_product.status = 1");
				...
				if($prices->rows) {
					foreach ($prices->rows as $pricesItem) {
						$pricesArr[] = $pricesItem['price'];
					}
			...
			if(!isset($this->request->get['min']) && !isset($this->request->get['max'])) {
				if(isset($data['min_price']) && isset($data['max_price'])) {
					$data['min_price_current'] = $data['min_price'];
					$data['max_price_current'] = $data['max_price'];
				} else {
					$data['min_price_current'] = 0;
					$data['max_price_current'] = 0;
				}
			} else {
				$data['min_price_current'] = $this->request->get['min'];
				$data['max_price_current'] = $this->request->get['max'];
			}
