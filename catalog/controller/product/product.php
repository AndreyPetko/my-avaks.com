...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		if($this->config->get('occm_status')) $this->document->addScript($this->url->link('module/occm/js','','SSL'));
		...
		if ($product_info) {
			...
			if(isset($this->session->data['compare'])) {
				$data['in_compare'] = in_array($this->request->get['product_id'], $this->session->data['compare']) ? 1 : 0;
			} else {
				$data['in_compare'] = 1;
			}
			...
			if ($this->customer->isLogged()) {
				...
				$result = $this->db->query("SELECT product_id FROM oc_customer_wishlist WHERE customer_id = " . $customerId);
