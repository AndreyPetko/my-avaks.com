<?php
// Heading
$_['heading_title']       = 'Логинза (платный модуль)';

// Text
$_['text_module']         = 'Модули';
$_['text_success']        = 'Настройки модуля обновлены!';
$_['text_content_top']    = 'Содержание шапки';
$_['text_content_bottom'] = 'Содержание подвала';
$_['text_column_left']    = 'Левая колонка';
$_['text_column_right']   = 'Правая колонка';

$_['text_showtype_notice']	= 'Модальное окно не открывается на странице /index.php?route=checkout/checkout';

$_['entry_save_to_addr']	= 'Сохранять данные логинзы';
$_['text_customer_addr']	= 'В данные пользователя и в адрес пользователя';
$_['text_customer_only']	= 'Только в данные пользователя';

$_['entry_shop_folder']		= 'Директория магазина из домена:<br><i style="font-weight: normal;">
Если в домене Вашего магазина есть директория, например: http://site.ru/shop/ то укажите название директории ( "shop" ) в этом поле.
Впишите значение без знаков слэш ("/"). Если же в домене вашего магазина нет директории, например htttp://site.ru - то оставьте это поле пустым.
</i>';

$_['button_add_module']	= 'Добавить модуль';
$_['button_remove']	= 'Удалить';

$_['entry_version'] = 'Версия модуля:';



$_['entry_dobortype']	= 'Когда делать добор данных';
$_['entry_dobortype_one']	= 'Только при первой авторизации';
$_['entry_dobortype_every']	= 'При каждой авторизации';


$_['entry_popup_name'] = 'Заголовок всплывающиго окна';
$_['text_popup_notice'] = 'Окно всплывает поверх страницы. Если пользователь закрывает Окно, 
то оно больше не появляется';
$_['tab_popup']	     = 'Всплывающие окно';
$_['loginza2_popup_name_default'] = 'Авторизация через соц.сети';
		
		
		
$_['col_sort_order']	     = 'Порядок отображения';

/* start update: a1 */ 
$_['entry_confirm_data']	     = 'Добор данных после авторизации';
$_['entry_confirm_data_notice']	 = 'После авторизации пользователь увидит модальное окно с 
формой где он должен будет ввести данные. Если добор для всех полей отключен, то модальное окно не появится и пользователь будет авторизован сразу.';
$_['entry_confirm_firstname'] 	 = 'Добор поля "Имя":';
$_['entry_confirm_lastname'] 	 = 'Добор поля "Фамилия":';
$_['entry_confirm_email'] 		 = 'Добор поля "E-mail":';
$_['entry_confirm_phone'] 		 = 'Добор поля "Телефон":';
$_['text_confirm_disable'] 		 = 'Отключить добор';
$_['text_confirm_none'] 		 = 'Включить добор если данные не получены';
$_['text_confirm_allways'] 		 = 'Включить добор даже если данные получены (полученные данные будут введены в поле по умолчанию)';
/* end update: a1 */ 

$_['entry_widget_after']     = 'После авторизации:';
$_['text_widget_after_show'] = 'Отображать ссылки аккаунта';
$_['text_widget_after_hide'] = 'Скрывать виджет';

/* start update: c1 */ 

$_['loginza2_widget_name_default'] = 'Авторизация через соц.сети';

$_['entry_widget_name']	  = 'Заголовок виджета';
$_['entry_confirm_company'] 	 = 'Добор поля "Компания":';
$_['entry_confirm_address_1'] 	 = 'Добор поля "Адрес":';
$_['entry_confirm_postcode'] 	 = 'Добор поля "Почтовый индекс":';
$_['entry_confirm_city'] 	 	 = 'Добор поля "Город":';
$_['entry_confirm_zone'] 		 = 'Добор поля "Регион":';
$_['entry_confirm_country'] 	 = 'Добор поля "Страна":';
$_['text_required'] 	 		 = 'Обязательное поле';

$_['entry_showtype']      = 'После нажатия на иконку:';
$_['entry_widget_showtype']    = 'После нажатия на иконку:';
$_['text_showtype_window'] = 'Открывать модальное окно';
$_['text_showtype_redirect'] = 'Перенаправлять на сайт логинзы';

/* end update: c1 */

// Entry
$_['entry_admin']         = 'Только администраторы:';
$_['entry_layout']        = 'Схема:';
$_['entry_position']      = 'Расположение:';
$_['entry_status']        = 'Статус:';
$_['entry_sort_order']    = 'Порядок сортировки:';

$_['entry_google']		  = 'Google';
$_['url_google']		  = 'http://gmail.com';
$_['data_google']		  		  = '';

$_['entry_yandex']		  = 'Яндекс';
$_['url_yandex']		  = 'http://yandex.ru';
$_['data_yandex']		  		  = '';

$_['entry_mailruapi']	  = 'OPENID@MAIL.RU';
$_['url_mailruapi']		  = 'http://openid.mail.ru/';
$_['data_mailruapi']		  		  = '';

$_['entry_mailru']	  	  = 'Mail.ru';
$_['url_mailru']		  = 'http://mail.ru/';
$_['data_mailru']		  		  = '';

$_['entry_vkontakte']	  = 'ВКонтакте';
$_['url_vkontakte']		  = 'http://vk.com/';
$_['data_vkontakte']		  		  = '';

$_['entry_facebook']	  = 'FaceBook';
$_['url_facebook']		  = 'http://facebook.com/';
$_['data_facebook']		  		  = '';

$_['entry_odnoklassniki'] = 'Одноклассники';
$_['url_odnoklassniki']	  = 'http://odnoklassniki.ru/';
$_['data_odnoklassniki']		  		  = '';

$_['entry_livejournal']	  = 'Живой Журнал';
$_['url_livejournal']	  = 'http://livejournal.com/';
$_['data_livejournal']		  		  = '';

$_['entry_twitter']	      = 'Twitter';
$_['url_twitter']		  = 'http://twitter.com/';
$_['data_twitter']		  		  = '';

$_['entry_linkedin']	  = 'LinkedIn';
$_['url_linkedin']		  = 'http://linkedin.com/';
$_['data_linkedin']		  		  = '';

$_['entry_loginza']	  	  = 'Loginza';
$_['url_loginza']		  = 'http://loginza.ru/';
$_['data_loginza']		  		  = '';

$_['entry_myopenid']	  = 'MyOpenid';
$_['url_myopenid']		  = 'http://myopenid.com/';
$_['data_myopenid']		  		  = '';

$_['entry_webmoney']	  = 'WebMoney';
$_['url_webmoney']		  = 'http://webmoney.ru/';
$_['data_webmoney']		  		  = '';

$_['entry_rambler']	 	  = 'Rambler';
$_['url_rambler']		  = 'http://rambler.ru/';
$_['data_rambler']		  		  = '';

$_['entry_flickr']	 	  = 'Flickr';
$_['url_flickr']		  = 'http://flickr.com/';
$_['data_flickr']		  		  = '';

$_['entry_lastfm']	 	  = 'LastFM';
$_['url_lastfm']		  = 'http://last.fm/';
$_['data_lastfm']		  		  = '';

$_['entry_verisign']	  = 'VeriSign';
$_['url_verisign']		  = 'http://verisign.com/';
$_['data_verisign']		  		  = '';

$_['entry_aol']	  		  = 'AOL';
$_['url_aol']		      = 'http://aol.com/';
$_['data_aol']		  		  = '';

$_['entry_steam']	  	  = 'Steam';
$_['url_steam']		      = 'http://store.steampowered.com/';
$_['data_steam']		  		  = '';

$_['entry_openid']	  	  = 'OpenID';
$_['url_openid']		  = 'http://openid.net/';
$_['data_openid']		  		  = '';

$_['col_socnet']		  = 'Соц.сети';
$_['col_data']			  = 'Возвращаемые данные';
$_['col_enable']		  = 'Подключить';

$_['entry_format']		  = 'Отображать значки Логинзы в формате:';
$_['text_format_table']	  = 'Крупные значки таблицей';
$_['text_format_button']  = 'Кнопка Логинзы';
$_['text_format_icons']   = 'Маленькие иконки';

$_['entry_label']	      = 'Заголовок перед иконками';

$_['entry_show_standart_auth'] = 'Отображать в виджете стандартную авторизацию';
$_['entry_widget_format'] = 'Отображать значки в формате:';


$_['button_save_and_go']  = 'Сохранить и выйти';
$_['button_save_and_stay']  = 'Сохранить и остаться';


$_['entry_widget_header']    = 'Информация о виджете';
$_['entry_widget_notice']    = 'Чтобы получить информацию о виджете нужно будет зарегистрировать виджет на сайте Логинзы.
Если Вы не зарегистрируете виджет, то он будет работать и так. Но Вы не сможете использовать "Безопасный режим проверки token". (В документации Логинзы его рекомендуют использовать)<br>
<p>Если Вы решили зарегистрировать виджет:</p>
<ul>
	<li><a href="http://loginza.ru" target=_blank>Перейдите на сайт Логинзы</a></li>
	<li>Зарегистрируйтесь/Авторизуйтесь там.</li>
	<li>Перейдите в раздел "Мой виджет Логинза" и создайте виджет</li>
	<li>Подтвердите права на сайт (чтобы встроить код в тэг &lt;head&gt; перейдите в "Система" -> "Настройки" -> Магазин -> Вкладка "Сервер" и введите код в поле "Код Google Analytics")</li>
	<li>Зайдите в раздел "Настройка" виджета, который появится после того как Вы подтвердите права.</li>
	<li>Перенесите параметры ID и "Секретный ключ" в поля ниже</li>
</ul>';


$_['text_frame']	  = 'Информация ниже отображается во фрэйме. Список вопросов периодически пополняется, так же здесь могут появляться объявления.';

$_['text_contact']	  = '<p>Если Вы не нашли ответов на возникшие у Вас вопросы - свяжитесь с разработчиком модуля:</p>
			<p>Скайп: kin154</p>
			<p>E-mail: internetstartru@gmail.com</p>			
			<p>Я бываю на работе как правило с 12 до 20 по мск, по будням. По выходным - иногда тоже бываю на связи.</p>
			<p>---------------<br>
			С уважением,<br>
			программист Константин Петров.</p>
			<p>Приятной работы!</p>
			-------------------------------------------------
			<p><b>ДРУГИЕ МОДУЛИ АВТОРА:</b></p>
			<p>Робокасса 20 методов<br>
<a href="http://opencartforum.ru/files/file/305-robokassa-20-sposobov-oplaty/" target="blank">http://opencartforum.ru/files/file/305-robokassa-20-sposobov-oplaty/</a></p>


<p>Авторизация через Вконтакте, Facebook, Одноклассники, Twitter<br>
<a href="http://opencartforum.ru/files/file/741-avtorizatciia-cherez-vkontakte-facebook-odnoklassniki-twitte/"   target="blank">http://opencartforum.ru/files/file/741-avtorizatciia-cherez-vkontakte-facebook-odnoklassniki-twitte/</a></p>

<p>EMS Почта России<br>
<a href="http://opencartforum.ru/files/file/306-ems-pochta-rossii/" target="blank">http://opencartforum.ru/files/file/306-ems-pochta-rossii/" target="blank</a></p>

<p>Меню категорий разворачивающееся с эффектом скольжения<br>
<a href="http://opencartforum.ru/files/file/472-meniu-kategorii-razvorachivaiuscheesia-s-effektom-s/" target="blank">http://opencartforum.ru/files/file/472-meniu-kategorii-razvorachivaiuscheesia-s-effektom-s/</a></p>
			';


$_['entry_sig'] 		 = 'Секретный ключ';
$_['entry_id'] 		 = 'ID';
$_['entry_safemode']  = 'Безопасный режим проверки token';

$_['filter_header']		  = 'Управление доступными для авторизации Соц.сетями';
$_['filter_notice']		  = 'Разные соц.сети при авторизации пользователя возвращает разный набор информации о пользователе.
Эта информация будет отображаться в панели администрирования в карточках заказа и карточках покупателя.
Поэтому, если Вы хотите иметь больше информации, есть смысл исключить те соц.сети которые возвращают мало информации.
Если же для Вас не важна информация о пользователе, получаемая через соц.сети, то имеет смысл включить все соц.сети.';

$_['text_country'] 		 = 'Страна';
$_['text_regions'] 		 = 'Регионы РФ';
$_['entry_default'] 	 = 'Соц.Сеть по умолчанию, которая будет предложена пользователю в виджете.';

$_['entry_widget_layout']        = 'Страница:';
$_['entry_widget_position']      = 'Расположение:';
$_['entry_widget_status']        = 'Статус:';
$_['entry_widget_sort_order']    = 'Порядок сортировки:';

$_['tab_general'] 	 = 'Общие настройки';
$_['tab_dobor'] 	 = 'Добор данных';
$_['tab_socseti'] 	 = 'Провайдеры авторизации';
$_['tab_widget'] 	 = 'Виджеты';
$_['tab_admin'] 	 = 'Админка';
$_['tab_design']     = 'Дизайн';

$_['tab_support'] 	 = 'Тех.поддержка';


$_['col_link'] 	 = 'Ссылка на аккаунт';
$_['col_nickname'] 	 = 'Логин';
$_['col_email'] 	 = 'E-mail';
$_['col_gender'] 	 = 'Пол';
$_['col_photo'] 	 = 'Фото';
$_['col_name'] 	 = 'ФИО';
$_['col_dob'] 	 = 'Дата рождения';
$_['col_work'] 	 = 'Работа';
$_['col_address'] 	 = 'Адрес';
$_['col_phone'] 	 = 'Телефон';
$_['col_im'] 	 = 'Skype,ICQ';
$_['col_biography'] 	 = 'Доп.информация';

$_['entry_admin_header'] 	 = 'Отображать информацию о способе авторизации в АДМИНКЕ';
$_['entry_admin_customer']	 = 'В карточке покупателя';
$_['entry_admin_customer_list']	 = 'В списке покупателей (колонка "Провайдер")';
$_['entry_admin_order']	 	 = 'В карточке заказа';
$_['entry_admin_order_list'] = 'В списке заказов (колонка Провайдер)';


$_['text_design_notice2'] = 'Далее перечислены файлы в которых лежит дизайн разных элементов модуля';

	  $_['text_design_col_element'] = "Элемент дизайна";
	  $_['text_design_col_file'] = "Адрес файла";
	  $_['text_design_col_comment'] = "Комментарий";
	  $_['text_design_row_loginza2_account'] = "Виджет логинзы на странице авторизации";
	  $_['text_design_notice'] = "<b><font color=red>Внимание!</font></b> после изменения данного файла нажмите &quot;Сохранить&quot; в настройках модуля. В этот момент данные файла сохраняются в базу и при отображении пользователю - берутся уже из базы, а не из файла.";
	  $_['text_design_row_loginza2_checkout'] = "Виджет логинзы на странице стандартного оформления заказа";
	  $_['text_design_row_loginza2_simple'] = "Виджет логинзы на странице оформления заказа через модуль Simple";
	  $_['text_design_row_loginza2_simplereg'] = "Виджет логинзы на странице регистрации через модуль Simple";
	  $_['text_design_row_loginza2_popup'] = "Всплывающие окно авторизации (настраивается из вкладки &quot;Всплывающее окно&quot;)";
	  $_['text_design_row_loginza2_confirm'] = "Всплывающие окно добора данных (внешняя часть)";
	  $_['text_design_row_loginza2_frame'] = "Форма отображаемая в окне добора ";
	  $_['text_design_row_loginza2_frame_success'] = "Страница с сообщением об успешной регистрации в окне добора";
	  $_['text_design_row_module_loginza2'] = "Виджет авторизации через логинзу (настраивается из вкладки &quot;Виджеты&quot;)";
	  
// Error
$_['error_permission']    = 'У Вас нет прав для управления этим модулем!';
?>